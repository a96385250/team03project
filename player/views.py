...
def rankingavg(request):
    datas=[]
    datasql = "SELECT teamid,playername,avg from players where ab > 308 ORDER BY avg DESC LIMIT 5"
...
def rankingera(request):
    datas=[]
    datasql = "select teamid,playername,era from players  where era >0.00 and ip >118.1 order by era limit 5;"
    playersdata=playersdbdata.read(datasql)
    for playerdata in  playersdata:
        if playerdata[0] == 1:
            playerteam = "Lamigo"
